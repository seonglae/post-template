    ...
    doi_batch: [
...

      {
        head: [
          ...
          { depositor: [{ depositor_name: data.journal.publisherName }, { email_address: data.journal.publisherEmail }] },
            ...
            journal: [
              {
                journal_metadata: [
                  ...
                  { doi_data: [{ doi: data.journal.doi }, { resource: data.journal.url }] },
                ],
              },

              {
                journal_issue: [
                  { publication_date: [{ month: date.getMonth() + 1 }, { year: date.getFullYear() }] },
                  { journal_volume: [{ volume: data.volume }] },
                  { issue: data.issue },
                ],
              },

              {
                journal_article: [
                  ...
                  {
                    contributors: data.authors.map((author, ind) => ({
                      person_name: [
                        {
                          _attr: {
                  ...
                  { publisher_item: [{ item_number: data.doi }] },
                  {
                    doi_data: [
                      { doi: data.doi },
                      //{timestamp: ""},
                  ...
                  { citation_list: data.citations.map(key => citation_xml(key, data.bibliography[key])) },
                ],
              },
            ],
          },
